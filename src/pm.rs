...
pub fn install_package(pkg_name: &str) -> Result<(), String> {
    ...
    if pkg_name.contains('/') {
        ...
        Err(format!("Unknown package registry for '{}'. Use 'user/repo' for GitHub.", pkg_name))
    }
}
...
fn fetch_git_dep(name: &str, url: &str, tag: Option<&str>) -> Result<(), String> {
    ...
    if target_dir.exists() {
        ...
        if !out.status.success() {
            return Err(format!("Failed to clone {}: {}", name, String::from_utf8_lossy(&out.stderr)));
        }
    }
    ...
    if !lib_path.exists() {
        ...
        if alt.exists() {
            fs::write(&proxy_path, format!("import \"{}\" as _pkg; export _pkg;", alt.display())).map_err(|e| e.to_string())?;
        }
    } else {
        fs::write(&proxy_path, format!("import \"{}\" as _pkg; export _pkg;", lib_path.display())).map_err(|e| e.to_string())?;
    }
